...
## registered as `reject_standard_byte_subword_child` (this file, renamed): before S3(b), x86_64 handed
## the child to the WORD-per-field constructor while aarch64/riscv64/wat recursed through their own
## byte-precise writer, so no single offset was right on all four and S3(a) made all four refuse.
...
## CLAYOUT S4 subsequently makes `Small` itself a standard BYTE-tier type; this fixture therefore also
## locks the recursive image path rather than relying on the old word fallback for the child.
##
## The pre-S4 word model and the §6.1 model gave DIFFERENT answers here (audit §7 risk 4): `inner` is
## at byte 8 either way, but `b` is at child byte 2 (§6.1) versus child byte 8 (one word per field).
## Note `inner` IS 8-ALIGNED — that is what defeated S3(a)'s `(bo / 8) * 8 != bo` guard, which could
## not see the defect at all because `bo` was 8.
##
