...
## member requires the scope, and `v := unchecked (x.u)` re-emitted WITHOUT the parens became
## `unchecked x.u`, which re-parses as `(unchecked x).u` — the member read is back in checked mode and
