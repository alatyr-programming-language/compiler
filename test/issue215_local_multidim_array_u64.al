## Issue #215 failure-first reproducer: a local [[u64; 2]; 2] must preserve
...
main := fn() -> u64 {
  mut xs : [[u64; 2]; 2] = [[1, 2], [3, 4]]
  if xs[0][0] != 1 { return 1 }
  if xs[0][1] != 2 { return 2 }
  if xs[1][0] != 3 { return 3 }
  if xs[1][1] != 4 { return 4 }
  xs[1][0] = 9
  if xs[0][0] != 1 { return 1 }
  if xs[0][1] != 2 { return 2 }
  if xs[1][0] != 9 { return 3 }
  if xs[1][1] != 4 { return 4 }
  42
}
