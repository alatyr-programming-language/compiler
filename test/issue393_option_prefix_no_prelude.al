...
## trigger in that block checks the LEADING word boundary; the option-type trigger had no
## TRAILING one, so `Option_marker_unused` — a binding declared here, never read, and of no interest
...
## the parent compiler this file BUILDS and RUNS 42, and the very same file with line 22 deleted is
...
##
## Its sibling `test/issue393_result_prefix_no_prelude.al` is the same measurement on the other
## word; `test/bare_option_result.al` is the direction that must NOT change — a genuine bare mention,
## in every spelling a real use writes, still pulls the prelude and still runs 42.
Option_marker_unused := 0
