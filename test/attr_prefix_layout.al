...
## word: `@packed` newline `Pk := struct { a : u8, b : u16, c : u32 }` used to lay out 24 bytes where the
## value-position `Pk := @packed struct { … }` lays out 7. A declared layout silently replaced by a
## different one is the forbidden failure, so both spellings are pinned here. The un-attributed
## control also exercises CLAYOUT S4: direct scalar fields use their natural §6.1 byte layout (8 bytes
## here), while the packed declaration remains a 7-byte no-padding layout.
...
## The body also checks the natural one-byte alignment of the single-u8 control. (These notes live in
## the HEADER, not in the body: `fmt` cannot retain an in-body comment,
