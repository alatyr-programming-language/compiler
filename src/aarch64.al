...
## Issue #441 — the `deref(p) = v` STORE through a pointer whose pointee width is known. `x1` holds the
## byte address and `x0` the value; width 8 is the pre-existing `str x0, [x1]`, byte-for-byte.
##
...
## `src/riscv64.al` contain no `@repr` handling), so an enum local's tag occupies a whole word. With the
...
a64_deref_store_width_x1 := fn(width : usize, in out sb : rt::StrBuf) {
  if width == 8 { push_str(sb, "  str x0, [x1]\n") }
  else {
    push_str(sb, "  brk #0 // located: deref-store through a ")
    push_int(sb, i64(width))
    push_str(sb, "-byte pointee needs the @repr/sub-word byte image (aarch64)\n")
  }
}
...
## annotation to find at all, so `a64_local_ann_signed` above answers "not proven" and an enclosing
...
## Structurally the mirror image of `a64_operand_unsigned`'s `Bin` arm, down to the operator list;
  ...
  match deref(v) {
    Expr::Bin(op, bl, br) => {
      if op == 16 or op == 17 or op == 18 or op == 19 or op == 29 or op == 34 or op == 35 or op == 36 {
        sl := a64_operand_signed_dep(bl, params_head, body_head, src, a, dep)
        sr := a64_operand_signed_dep(br, params_head, body_head, src, a, dep)
        if sl and sr { r = true }
...
## backend's `:=` locals are function-scoped, `a64_local_rhs` resolves a name to its FIRST flat
...
a64_operand_signed_dep := fn(e : ptr(Expr), params_head : ptr(mut Param), body_head : ptr(mut Stmt), src : ptr(u8), a : rt::Arena, dep : i64) -> bool {
  ...
  match deref(e) {
    Expr::Var(s, n) => {
      ...
      if r == false { rhs := a64_local_rhs(body_head, src, s, n, a); if unchecked bitcast(usize, rhs) != 0 { if a64_shift_call_signed(rhs, params_head, body_head, src, a, dep + 1) { r = true } } }
...
## The UNSIGNED-direction twin of `a64_hole_index_signed` below. An `xs[i]` read carries no
...
## array FIELD of a struct (`r.u[0]`), whose base is an `Expr::Field` that only `a64_place_ty` can
...
a64_index_elem_unsigned := fn(bse : ptr(Expr), params_head : ptr(mut Param), body_head : ptr(mut Stmt), src : ptr(u8), a : rt::Arena) -> bool {
  ...
  mut r := false
  if bn != 0 {
    an := a64_local_arrty_span(body_head, src, bs, bn, a)
    if arrty_elem_unsigned(src, an.s, an.n) { r = true }
    if r == false { sp := a64_slice_param_elem_span(params_head, src, bs, bn) ; if sp.n != 0 { if scalar_name_is_unsigned(src, sp.s, sp.n) { r = true } } }
    ## A `Slice(T)` LOCAL. Its element type is written ONLY in the annotation at its declaration,
    ## so neither the fixed-array span above nor the param lookup can see it. `a64_slice_elem_span`
    ...
    ## `a64_local_ann_span` already walks to the DECLARATION and answers its annotation span, so the
    ...
    if r == false {
      la := a64_local_ann_span(body_head, src, bs, bn, a)
      if la.n > 6 {
...
a64_operand_unsigned := fn(e : ptr(Expr), params_head : ptr(mut Param), body_head : ptr(mut Stmt), src : ptr(u8), a : rt::Arena) -> bool {
  ...
  match deref(e) {
    ...
    Expr::Index(bse, ix) => { if a64_index_elem_unsigned(bse, params_head, body_head, src, a) { r = true } }
    Expr::Num | Expr::BoolLit | Expr::If | Expr::Match | Expr::StructLit | Expr::Field
...
## A preserved bare narrow scalar bitcast must restore the destination word just like a conversion.
...
## function owns only the aarch64 encoding. A SIGNED target reuses `a64_emit_narrow`; every other
## narrow target zero-extends, which is byte-for-byte what `a64_emit_narrow` emits for `uN`.
a64_emit_bitcast_narrow := fn(name : str, in out sb : rt::StrBuf) {
  w := bitcast_narrow_bytes(name)
  if bitcast_narrow_is_signed(name) { a64_emit_narrow(name, sb) }
  else if w == 1 { push_str(sb, "  uxtb x0, w0\n") }
  else if w == 2 { push_str(sb, "  uxth x0, w0\n") }
  else if w == 4 { push_str(sb, "  mov w0, w0\n") }
}
...
emit_a64_expr := fn(e : ptr(Expr), in out sb : rt::StrBuf, a : rt::Arena, src : ptr(u8), params_head : ptr(mut Param), pcount : i64, body_head : ptr(mut Stmt), decls : ptr(rt::Vec), bind_head : ptr(mut Bind), bind_base : i64) {
  match deref(e) {
    ...
    ## keeps the byte-identical `ldr x0, [x0]`. Without this the pointer identity added to the `Bitcast`
    ...
    Expr::Deref(inner) => {
      ...
      a64_std_load_width_x0(deref_bitcast_pointee_bytes(inner, body_head, src), deref_bitcast_pointee_signed(inner, body_head, src), sb)
    }
    Expr::Bin(op, l, r) => {
    ...
    ## with an anonymous `brk`, so `bitcast(ptr(mut bits8), n)` — an integer→pointer reinterpret that
    ## x86_64 runs — trapped on aarch64 while x86_64 answered 42.
    ##
    ...
    ## machine word, this backend has no aggregate-value bitcast path (`emit_a64_expr` delivers one
    ## word in x0), and emitting the inner value here would answer `b := bitcast(B, a)` with a WRONG
    ## value where there had been a trap — which I11 forbids. The enclosing function label above the
    ## trap locates it in the emitted GAS.
    Expr::Bitcast(inner, ts, tl) => {
      if bitcast_target_is_narrow_scalar(src, ts, tl) {
        emit_a64_expr(inner, sb, a, src, params_head, pcount, body_head, decls, bind_head, bind_base)
        a64_emit_bitcast_narrow(str_at((src + ts), tl), sb)
      } else if bitcast_target_is_pointer(src, ts, tl) {
        emit_a64_expr(inner, sb, a, src, params_head, pcount, body_head, decls, bind_head, bind_base)
      } else {
        push_str(sb, "  brk #0 // unsupported bitcast to aggregate target `")
        push_str(sb, str_at((src + ts), tl))
        push_str(sb, "` in value position (aarch64 has no aggregate-value bitcast path)\n")
      }
    }
...
## ISSUE #448 — `pe` NAMES AN ENUM PARAM. §8 piece 3 passes an enum BY REFERENCE, so an enum param's
## frame slot holds a POINTER to the caller's {disc, payload…} block, never the discriminant. Every
## one-word struct-field / return-register store below emitted that pointer straight into the field, so
## `h := Holder(t = v)` wrote an ADDRESS where the discriminant belongs; the `match h.t` dispatch that
## reads the field then compared the address against 0, 1, 2, matched no arm and answered the WILDCARD —
## a silent wrong value on a shape whose every other spelling is a fail-loud `brk`. Load the block
...
a64_emit_enum_param_disc := fn(pe : ptr(Expr), in out sb : rt::StrBuf, a : rt::Arena, src : ptr(u8), params_head : ptr(mut Param), body_head : ptr(mut Stmt), decls : ptr(rt::Vec)) -> bool {
  ...
  if a64_local_enum_nl(body_head, src, vns, vnl, a) != 0 { return false }
  pidx := param_find(params_head, src, vns, vnl, a)
  if pidx < 0 { return false }
  if a64_param_enum_nl(params_head, src, vns, vnl, decls) == 0 { return false }
  push_str(sb, "  ldr x0, [x29, #") ; push_int(sb, 16 + pidx * 8) ; push_str(sb, "]\n  ldr x0, [x0, #0]\n")
  true
}

## ISSUE #448, the MULTI-WORD half. Copy the FULL {disc, payload…} width of an enum PLACE `pe` into the
## frame at byte offset `off` and report the words written; 0 = `pe` is not an enum place and the caller
## keeps its one-word fallback. Two cases, both previously written as ONE word by the scalar fallback:
##   * an enum PARAM — the slot holds the by-reference POINTER (see above), so nothing about the stored
##     word was even a discriminant;
##   * an enum LOCAL wider than one word — its frame slot IS the block, so word 0 landed correctly and
##     every PAYLOAD word was dropped: `Boxed(p = e)` for `e := Pay.B(5)` matched arm `B` and then read
...
a64_store_enum_place_at := fn(pe : ptr(Expr), off : i64, in out sb : rt::StrBuf, a : rt::Arena, src : ptr(u8), params_head : ptr(mut Param), pcount : i64, body_head : ptr(mut Stmt), decls : ptr(rt::Vec)) -> i64 {
  ...
  vnl := ex_var_nl(pe)
  if vnl == 0 { return 0 }
  lenl := a64_local_enum_nl(body_head, src, vns, vnl, a)
  if lenl != 0 {
    loff := a64_local_off(body_head, src, vns, vnl, pcount, a, decls)
    lens := a64_local_enum_ns(body_head, src, vns, vnl, a)
    ...
    ## at block word 1 and moves exactly `union_words` words. The enum count below is one word too wide
    ## AND one word out of place, silently: the corruption lands in the field AFTER the union field.
    ## A ONE-WORD union must fire too — the scalar fallback would store the block's DISCRIMINANT, not the
    ## payload — so there is no `lw < 2` decline here.
    if lower_layout::is_union_decl(decls, src, lens, lenl) {
      if loff < 0 { return 0 }
      ulw := i64(lower_layout::union_words(decls, src, lens, lenl, a))
      mut uk := 0
      while uk < ulw {
        push_str(sb, "  ldr x0, [x29, #") ; push_int(sb, loff + 8 + uk * 8) ; push_str(sb, "]\n  str x0, [x29, #") ; push_int(sb, off + uk * 8) ; push_str(sb, "]\n")
        uk = uk + 1
      }
      return ulw
    }
    ...
    mut k := 0
    while k < lw {
      push_str(sb, "  ldr x0, [x29, #") ; push_int(sb, loff + k * 8) ; push_str(sb, "]\n  str x0, [x29, #") ; push_int(sb, off + k * 8) ; push_str(sb, "]\n")
      k = k + 1
    }
    return lw
  }
  pidx := param_find(params_head, src, vns, vnl, a)
  penl := a64_param_enum_nl(params_head, src, vns, vnl, decls)
  if pidx < 0 or penl == 0 { return 0 }
  pens := a64_param_enum_ns(params_head, src, vns, vnl, decls)
  ...
  if lower_layout::is_union_decl(decls, src, pens, penl) {
    ...
    mut uj := 0
    while uj < upw {
      push_str(sb, "  ldr x0, [x2, #") ; push_int(sb, 8 + uj * 8) ; push_str(sb, "]\n  str x0, [x29, #") ; push_int(sb, off + uj * 8) ; push_str(sb, "]\n")
      uj = uj + 1
    }
    return upw
  }
  ...
  mut j := 0
  while j < pw {
    push_str(sb, "  ldr x0, [x2, #") ; push_int(sb, j * 8) ; push_str(sb, "]\n  str x0, [x29, #") ; push_int(sb, off + j * 8) ; push_str(sb, "]\n")
    j = j + 1
  }
  pw
}
...
a64_store_enum_place_atptr := fn(pe : ptr(Expr), off : i64, in out sb : rt::StrBuf, a : rt::Arena, src : ptr(u8), params_head : ptr(mut Param), pcount : i64, body_head : ptr(mut Stmt), decls : ptr(rt::Vec)) -> i64 {
  ...
  if lenl != 0 {
    loff := a64_local_off(body_head, src, vns, vnl, pcount, a, decls)
    lens := a64_local_enum_ns(body_head, src, vns, vnl, a)
    ## ISSUE #497, the POINTER-relative twin of the raw-union local arm in `a64_store_enum_place_at`.
    if lower_layout::is_union_decl(decls, src, lens, lenl) {
      if loff < 0 { return 0 }
      ulw := i64(lower_layout::union_words(decls, src, lens, lenl, a))
      mut uk := 0
      while uk < ulw {
        push_str(sb, "  ldr x0, [x29, #") ; push_int(sb, loff + 8 + uk * 8) ; push_str(sb, "]\n  ldr x1, [sp]\n  str x0, [x1, #") ; push_int(sb, off + uk * 8) ; push_str(sb, "]\n")
        uk = uk + 1
      }
      return ulw
    }
    ...
    mut k := 0
    while k < lw {
      push_str(sb, "  ldr x0, [x29, #") ; push_int(sb, loff + k * 8) ; push_str(sb, "]\n  ldr x1, [sp]\n  str x0, [x1, #") ; push_int(sb, off + k * 8) ; push_str(sb, "]\n")
      k = k + 1
    }
    return lw
  }
  ...
  if pidx < 0 or penl == 0 { return 0 }
  pens := a64_param_enum_ns(params_head, src, vns, vnl, decls)
  ## ISSUE #497, the POINTER-relative twin of the raw-union param arm in `a64_store_enum_place_at`.
  if lower_layout::is_union_decl(decls, src, pens, penl) {
    upw := i64(lower_layout::union_words(decls, src, pens, penl, a))
    push_str(sb, "  ldr x2, [x29, #") ; push_int(sb, 16 + pidx * 8) ; push_str(sb, "]\n")
    mut uj := 0
    while uj < upw {
      push_str(sb, "  ldr x0, [x2, #") ; push_int(sb, 8 + uj * 8) ; push_str(sb, "]\n  ldr x1, [sp]\n  str x0, [x1, #") ; push_int(sb, off + uj * 8) ; push_str(sb, "]\n")
      uj = uj + 1
    }
    return upw
  }
  ...
  mut j := 0
  while j < pw {
    push_str(sb, "  ldr x0, [x2, #") ; push_int(sb, j * 8) ; push_str(sb, "]\n  ldr x1, [sp]\n  str x0, [x1, #") ; push_int(sb, off + j * 8) ; push_str(sb, "]\n")
    j = j + 1
  }
  pw
}
...
a64_store_enum_call_at := fn(pe : ptr(Expr), off : i64, in out sb : rt::StrBuf, a : rt::Arena, src : ptr(u8), params_head : ptr(mut Param), pcount : i64, body_head : ptr(mut Stmt), decls : ptr(rt::Vec), bind_head : ptr(mut Bind), bind_base : i64) -> i64 {
  ...
  ## word 0 = discriminant in x0, payload word k in x(k+1) (measured — a union return builds the same
  ## `{disc, payload…}` block). The struct FIELD is `field_words` = `union_words` PAYLOAD words at offset
  ## 0 with NO discriminant (spec Types §6.3), so the field takes x1..x(union_words) and the enum count
  ## below is one word too wide AND one word out of place — silently, in the field AFTER it. A ONE-WORD
  ## union must fire too: the scalar fallback would store x0, the DISCRIMINANT, not the payload.
  if lower_layout::is_union_decl(decls, src, cre.s, cre.n) {
    ucw := i64(lower_layout::union_words(decls, src, cre.s, cre.n, a))
    emit_a64_expr(pe, sb, a, src, params_head, pcount, body_head, decls, bind_head, bind_base)
    mut uk := 0
    while uk < ucw {
      push_str(sb, "  str x") ; push_int(sb, uk + 1) ; push_str(sb, ", [x29, #") ; push_int(sb, off + uk * 8) ; push_str(sb, "]\n")
      uk = uk + 1
    }
    return ucw
  }
...
a64_store_enum_call_atptr := fn(pe : ptr(Expr), off : i64, in out sb : rt::StrBuf, a : rt::Arena, src : ptr(u8), params_head : ptr(mut Param), pcount : i64, body_head : ptr(mut Stmt), decls : ptr(rt::Vec), bind_head : ptr(mut Bind), bind_base : i64) -> i64 {
  ...
  if lower_layout::is_union_decl(decls, src, cre.s, cre.n) {
    ...
    mut uk := 0
    while uk < ucw {
      push_str(sb, "  str x") ; push_int(sb, uk + 1) ; push_str(sb, ", [x9, #") ; push_int(sb, off + uk * 8) ; push_str(sb, "]\n")
      uk = uk + 1
    }
    return ucw
  }
...
emit_a64_store_payload_at := fn(pe : ptr(Expr), off : i64, in out sb : rt::StrBuf, a : rt::Arena, src : ptr(u8), params_head : ptr(mut Param), pcount : i64, body_head : ptr(mut Stmt), decls : ptr(rt::Vec), bind_head : ptr(mut Bind), bind_base : i64) -> i64 {
  ...
  ## one. But `field_words` reserves `union_words` for a union-typed field: the members OVERLAP AT OFFSET
  ## 0 and there is NO discriminant word. So the enum arm below is both ONE WORD TOO WIDE and one word
  ## out of place, and it is SILENT: every reader still resolves `field_word_offset`, so the wrong value
  ## surfaces in the field AFTER the union field. Write it the way x86_64's `emit_union_assign` writes
  ## it — the single member payload at word 0, `union_words` words reserved — and report `union_words`.
  ## A member that is not single-payload has no §6.3 layout at all (multi-payload is additive and
  ## undefined; a nullary member carries no value): a LOUD `brk` plus the reserved width, never a silent
  ## short store. Gated on `is_union_decl` → every enum keeps the byte-identical emit (fixpoint-neutral).
    if lower_layout::is_union_decl(decls, src, pens, penl) {
      ulw := i64(lower_layout::union_words(decls, src, pens, penl, a))
      ug := ex_enum_lit_args(pe)
      if ug == 0 {
        push_str(sb, "  brk #0 // a payload-free raw-union member has no field layout\n")
        return ulw
      }
      uga := deref(arg_p(ug))
      if uga.next != 0 {
        push_str(sb, "  brk #0 // a multi-payload raw-union member has no field layout\n")
        return ulw
      }
      _upw := emit_a64_store_payload_at(uga.e, off, sb, a, src, params_head, pcount, body_head, decls, bind_head, bind_base)
      return ulw
    }
...
emit_a64_store_payload_atptr := fn(pe : ptr(Expr), off : i64, in out sb : rt::StrBuf, a : rt::Arena, src : ptr(u8), params_head : ptr(mut Param), pcount : i64, body_head : ptr(mut Stmt), decls : ptr(rt::Vec), bind_head : ptr(mut Bind), bind_base : i64) -> i64 {
  ...
  if expr_is_enum_lit(pe) {
    pens := expr_enum_lit_ns(pe)
    penl := expr_enum_lit_nl(pe)
    ## ISSUE #497, the POINTER-relative twin of the raw-union arm in `emit_a64_store_payload_at`. Same
    ...
    if lower_layout::is_union_decl(decls, src, pens, penl) {
      ...
      _upw := emit_a64_store_payload_atptr(uga.e, off, sb, a, src, params_head, pcount, body_head, decls, bind_head, bind_base)
      return ulw
    }
    pvidx := variant_index(decls, src, pens, penl, expr_enum_variant_ns(pe), expr_enum_variant_nl(pe), a)
