...
## Flow §5.1). `src/aarch64.al` (#678) is the twin this file follows; the enforcement itself arrived
## with #557/PR #573, and the `ptr(T)` pointee resolution it leans on with #656/PR #662.
##
...
## `rv_emit_out_scalar_arg` is a NORMALIZER (every other form falls through to `emit_rv_expr` AFTER the
## match, so its group-arm body stays `{}`), and `emit_rv_expr` is the EMITTER, whose absorbed forms
## reach a located `ebreak` trap rather than a guessed value. A new `Expr` variant belongs in the group
## arm unless the accessor's own question can be TRUE of it — then it needs a real arm and a fixture.
## The reason is written once here instead of twenty-seven times; only the five arms whose reason is
## genuinely their own carry a note of their own.
##
## MEASURED on this file at `a65e873`, one arm at a time, the whole file restored between runs, with a
## compiler BUILT FROM THIS TREE — never the frozen seed, whose first duty in a census is to disagree
## with the tree it predates (`src/aarch64.al`'s first census was taken with the seed and scored 63/63
## accepted, because that seed predated the very fix it was being used to check):
##   * 27 REFUSED, rc 1, `alatyr: check: type mismatch at line <n> in riscv64`, naming the match head.
##     All 27 are `match deref(<expr>)` over `Expr`; all 27 are spelled out below.
##   * 25 ACCEPTED, rc 0, silent. All 25 are `match st` over `st := deref(stmt_p(Stmt, <h>))` — the
##     #660/#557 blind class, where a local inferred from a generic call adopts no enum type. They KEEP
##     their `_` and each is marked below. Spelling them out would buy the LOOK of enforcement and
##     nothing else, because a 22nd `Stmt` variant would still not be refused and nothing would hold
##     the written list current.
## This file never had `src/aarch64.al`'s PER-FILE blindness: `riscv64` sorts AFTER `ast` in module
## order, so `ptr(Expr)` always resolved here. That is the whole of the contrast #662's census
## recorded — `riscv64.al:590` refused where its byte-identical `aarch64.al` twin was silent — and it
## is why this census splits exactly along Expr/Stmt rather than along a per-file line.
##
## An OR-pattern arm's body is emitted once per ALTERNATIVE. That is invisible for the 24 `{}` bodies
## here, cheap for the two `return 0 - 1` accessors, and it USED to be fatal for a body holding a
## STRING LITERAL (#673: N `.rodata` definitions under one `.Lstr<m>_<k>` label, `as` refusing the
## self-build). PR #685 made the data walk visit a shared arm body once, and seed 0.2.3 (`a46be2c`)
## carries that fix, so `emit_rv_expr`'s group arm is written out here rather than deferred.
## ──────────────────────────────────────────────────────────────────────────────────────────────────
...
rv_slice_scan_has := fn(src : ptr(u8), end : usize, p : usize, want : str) -> bool {
  if p > end { return false }
  if want.len > end - p { return false }
  str_at((src + p), want.len) == want
}
rv_slice_scan_ws := fn(src : ptr(u8), end : usize, p : usize) -> bool {
  if p >= end { return false }
  c := str_at((src + p), 1)
  c == " " or c == "\n" or c == "\t" or c == "\r"
}
rv_slice_elem_span := fn(src : ptr(u8), s : usize, n : usize) -> CSpan {
  ...
  while p < end and rv_slice_scan_ws(src, end, p) { p = p + 1 }
  if rv_slice_scan_has(src, end, p, ":") == false { return CSpan(s = 0, n = 0) }
  p = p + 1
  while p < end and rv_slice_scan_ws(src, end, p) { p = p + 1 }
  if rv_slice_scan_has(src, end, p, "Slice(") == false { return CSpan(s = 0, n = 0) }
  es := p + 6
  mut ee := es
  while ee < end {
...
rv_cf_offset_value := fn(e : ptr(Expr), src : ptr(u8), decls : ptr(rt::Vec), a : rt::Arena) -> i64 {
  ...
  match deref(e) {
    ...
    ## The group arm's body is `return 0 - 1`, and an OR-pattern body is emitted once per ALTERNATIVE:
    ## 23 copies. Cheap here and semantically identical to the `_` it replaces; recorded so the size is
    ## not a surprise to the next reader.
...
rv_std_idx_path_bo := fn(e : ptr(Expr), body_head : ptr(mut Stmt), src : ptr(u8), a : rt::Arena, decls : ptr(rt::Vec)) -> i64 {
  ...
  base := expr_field_base(e)
  bt := rv_std_idx_path_ty(base, body_head, src, a, decls)
  rv_std_idx_path_bo(base, body_head, src, a, decls) + layout_field_offset_bytes(decls, src, bt.s, bt.n, expr_field_name_s(e), expr_field_name_l(e), a)
}

rv_std_idx_root_arr := fn(e : ptr(Expr)) -> ptr(Expr) {
  if ex_is_index(e) { return ex_index_base(e) }
  if ex_is_field(e) { return rv_std_idx_root_arr(expr_field_base(e)) }
  unchecked bitcast(ptr(Expr), 0)
}

rv_std_idx_root_idx := fn(e : ptr(Expr)) -> ptr(Expr) {
  if ex_is_index(e) { return ex_index_idx(e) }
  if ex_is_field(e) { return rv_std_idx_root_idx(expr_field_base(e)) }
  unchecked bitcast(ptr(Expr), 0)
}
...
rv_std_store_value := fn(pe : ptr(Expr), off : i64, ts : usize, tl : usize, wsize : usize, in out sb : rt::StrBuf, a : rt::Arena, src : ptr(u8), params_head : ptr(mut Param), pcount : i64, body_head : ptr(mut Stmt), decls : ptr(rt::Vec), bind_head : ptr(mut Bind), bind_base : i64) -> i64 {
  ...
  ## has no store here and still traps.
    if expr_is_struct_lit(pe) and std_struct_is_byte_writable(decls, src, ts, tl, a) { return rv_std_store_struct(pe, off, sb, a, src, params_head, pcount, body_head, decls, bind_head, bind_base) }
...
rv_std_store_value_atptr := fn(pe : ptr(Expr), off : i64, ts : usize, tl : usize, wsize : usize, in out sb : rt::StrBuf, a : rt::Arena, src : ptr(u8), params_head : ptr(mut Param), pcount : i64, body_head : ptr(mut Stmt), decls : ptr(rt::Vec), bind_head : ptr(mut Bind), bind_base : i64) -> i64 {
  ...
  bn := base_type_name(src, ts, tl)
  if struct_decl_of(decls, src, bn.s, bn.n) >= 0 {
    if expr_is_struct_lit(pe) and std_struct_is_byte_writable(decls, src, ts, tl, a) { return rv_std_store_struct_atptr(pe, off, sb, a, src, params_head, pcount, body_head, decls, bind_head, bind_base) }
...
rv_cf_mutable_value := fn(e : ptr(Expr), src : ptr(u8)) -> i64 {
  ...
  match deref(e) {
    ...
    ## `return 0 - 1` again, 23 copies — see the `rv_cf_offset_value` note above.
    Expr::Num | Expr::BoolLit | Expr::Var | Expr::Bin | Expr::If | Expr::Match | Expr::Call
      | Expr::StructLit | Expr::EnumLit | Expr::AddrOf | Expr::Deref | Expr::StrLit | Expr::ArrayLit
      | Expr::Index | Expr::Try | Expr::FloatLit | Expr::Slice | Expr::CompField | Expr::Unchecked
      | Expr::Lambda | Expr::FnRef | Expr::Bitcast | Expr::Loop => { return 0 - 1 }
...
rv_comp_range_bound := fn(e : ptr(Expr), decls : ptr(rt::Vec), src : ptr(u8)) -> i64 {
  ...
  match deref(e) {
    ...
    Expr::Field(b, fs, fl) => {
      ...
      rt := typeinfo_arg_span(src, fs, RV_SUB_GPS, RV_SUB_GPL, RV_SUB_ITS, RV_SUB_ITL, RV_SUB_GPS2, RV_SUB_GPL2, RV_SUB_ITS2, RV_SUB_ITL2, RV_SUB_GPS3, RV_SUB_GPL3, RV_SUB_ITS3, RV_SUB_ITL3)
    ...
    Expr::Bin(bop, bl, brr) => {
      lv := rv_comp_range_bound(bl, decls, src)
      rv := rv_comp_range_bound(brr, decls, src)
      mut dz := false
    ...
    ## THIS arm earns its own note, for #659's reason: the function's default is a REPRESENTABLE bound.
    ## `mut r := 0` is returned for every absorbed form, and the only caller — `emit_rv_stmts`'s
    ## `Stmt::CompForRange` unroll — commits to `while k < hi` with it, so an absorbed `hi` unrolls the
    ## body ZERO times on a clean compile with no diagnostic. #672 is the same shape already fixed for
    ## `Expr::Bin`; the enumeration below made the remaining absorbed forms a readable list rather than
    ## a `_`, and reading that list against the caller is what produced #695: `0 .. unchecked (3)` is
    ## measured at ZERO unrolls on all four backends where `0 .. 3` gives three. That repair is #695's,
    ## not this refactor's — this unit is byte-identical by construction and changes no behaviour.
...
rv_value_is_agg := fn(v : ptr(Expr)) -> bool { expr_is_struct_lit(v) or expr_is_enum_lit(v) or ex_is_array_lit(v) }
...
rv_place_ty := fn(e : ptr(Expr), body_head : ptr(mut Stmt), src : ptr(u8), a : rt::Arena, decls : ptr(rt::Vec)) -> CSpan {
  ...
  if ex_is_field(e) {
    bt := rv_place_ty(expr_field_base(e), body_head, src, a, decls)
    mut fok := false
    if bt.n != 0 {
...
rv_place_ok := fn(e : ptr(Expr), body_head : ptr(mut Stmt), src : ptr(u8), params_head : ptr(mut Param), pcount : i64, a : rt::Arena, decls : ptr(rt::Vec)) -> bool {
  ...
  if ex_is_field(e) {
    ...
    if rv_std_idx_path_ok(e, body_head, src, a, decls) { foffok = layout_field_offset_bytes(decls, src, bt.s, bt.n, expr_field_name_s(e), expr_field_name_l(e), a) >= 0 }
    if foffok { r = true }
    return r
  }
  if ex_is_index(e) {
...
emit_rv_place_addr := fn(e : ptr(Expr), in out sb : rt::StrBuf, a : rt::Arena, src : ptr(u8), params_head : ptr(mut Param), pcount : i64, body_head : ptr(mut Stmt), decls : ptr(rt::Vec), bind_head : ptr(mut Bind), bind_base : i64) {
  ...
  if isf {
    ...
    if rv_std_idx_path_ok(e, body_head, src, a, decls) { boff = layout_field_offset_bytes(decls, src, bt.s, bt.n, expr_field_name_s(e), expr_field_name_l(e), a) }
...
rv_cmp_unsigned := fn(l : ptr(Expr), r : ptr(Expr), params_head : ptr(mut Param), body_head : ptr(mut Stmt), src : ptr(u8), a : rt::Arena) -> bool {
  ...
  ul := bl == 2 or rv_operand_unsigned(l, params_head, body_head, src, a)
  ur := br == 2 or rv_operand_unsigned(r, params_head, body_head, src, a)
...
rv_emit_out_scalar_arg := fn(e : ptr(Expr), in out sb : rt::StrBuf, a : rt::Arena, src : ptr(u8), params_head : ptr(mut Param), pcount : i64, body_head : ptr(mut Stmt), decls : ptr(rt::Vec), bind_head : ptr(mut Bind), bind_base : i64) {
  ...
  match deref(e) {
    ...
    ## NORMALIZER, and its fallback is OUTSIDE the match (`if not done { emit_rv_expr(…) }` below), so
    ## unlike `a64_emit_out_scalar_arg` this group arm's body stays `{}` and forwards nothing itself.
    Expr::Num | Expr::BoolLit | Expr::Bin | Expr::If | Expr::Match | Expr::Call | Expr::StructLit
      | Expr::Field | Expr::EnumLit | Expr::AddrOf | Expr::Deref | Expr::StrLit | Expr::ArrayLit
      | Expr::Index | Expr::Try | Expr::FloatLit | Expr::Slice | Expr::CompField | Expr::Unchecked
      | Expr::Lambda | Expr::FnRef | Expr::Bitcast | Expr::Loop => {}
...
rv_int_const_expr := fn(e : ptr(Expr)) -> bool {
  ...
  match deref(e) {
    ...
    Expr::Unchecked(inner) => { if rv_int_const_expr(inner) { r = true } }
...
rv_direct_float_num := fn(e : ptr(Expr), src : ptr(u8), ns : usize, nl : usize, decls : ptr(rt::Vec), body_head : ptr(mut Stmt), params_head : ptr(mut Param), pcount : i64, a : rt::Arena, bind_head : ptr(mut Bind)) -> bool {
  ...
  if rv_local_off(body_head, src, dns, dnl, pcount, a, decls) >= 0 { return r }
...
rv_is_unit_enum_local := fn(e : ptr(Expr), body_head : ptr(mut Stmt), src : ptr(u8), a : rt::Arena, decls : ptr(rt::Vec)) -> bool {
  ...
  en := rv_local_enum_nl(body_head, src, ns, nl, a)
  if en == 0 { return false }
  es := rv_local_enum_ns(body_head, src, ns, nl, a)
  ...
  match deref(e) {
    Expr::Bin(op, l, rr) => {
      if ex_is_cmp_op(op) {
        ...
        if (op == 20 or op == 28) and rv_is_unit_enum_local(l, body_head, src, a, decls) and rv_is_unit_enum_local(rr, body_head, src, a, decls) { return false }
...
emit_rv_expr := fn(e : ptr(Expr), in out sb : rt::StrBuf, a : rt::Arena, src : ptr(u8), params_head : ptr(mut Param), pcount : i64, body_head : ptr(mut Stmt), decls : ptr(rt::Vec), bind_head : ptr(mut Bind), bind_base : i64) {
  match deref(e) {
    ...
    ## THE EMITTER. Its absorbed forms are the eight aggregate/string/lambda shapes this scalar kernel
    ## does not lower, and they reach a located `ebreak` (SIGTRAP -> exit 133 under qemu) rather than a
    ## guessed value — the file's stated contract. This body holds a STRING LITERAL, so until PR #685 an
    ## OR-pattern group arm here emitted eight `.rodata` definitions under one `.Lstr<m>_<k>` label and
...
## `1 + max_arity` words in the RETURN REGISTERS (word 0 = discriminant in a0, payload word k in a(k+1)),
## exactly as the enum-returning-call LOCAL bind below already reads them. The scalar fallback of
## `emit_rv_store_payload_at` stored ONE of them, so `Boxed(p = mk())` kept the right discriminant and read
## every PAYLOAD word out of a slot nothing wrote — 100 where 105 was due, while `e := mk() ; Boxed(p = e)`
...
rv_store_enum_call_at := fn(pe : ptr(Expr), off : i64, in out sb : rt::StrBuf, a : rt::Arena, src : ptr(u8), params_head : ptr(mut Param), pcount : i64, body_head : ptr(mut Stmt), decls : ptr(rt::Vec), bind_head : ptr(mut Bind), bind_base : i64) -> i64 {
  ...
  ## `rv_store_enum_place_at` declines a one-word enum LOCAL, so the emitted text stays byte-identical
  ## for every program that has one and only the dropped payload words are a change.
  if cw < 2 { return 0 }
  emit_rv_expr(pe, sb, a, src, params_head, pcount, body_head, decls, bind_head, bind_base)
  mut k := 0
  while k < cw {
    push_str(sb, "  sd a") ; push_int(sb, k) ; push_str(sb, ", ") ; push_int(sb, off + k * 8) ; push_str(sb, "(s0)\n")
    k = k + 1
  }
  cw
}
...
rv_store_enum_call_atptr := fn(pe : ptr(Expr), off : i64, in out sb : rt::StrBuf, a : rt::Arena, src : ptr(u8), params_head : ptr(mut Param), pcount : i64, body_head : ptr(mut Stmt), decls : ptr(rt::Vec), bind_head : ptr(mut Bind), bind_base : i64) -> i64 {
  ...
  mut k := 0
  while k < cw {
    push_str(sb, "  sd a") ; push_int(sb, k) ; push_str(sb, ", ") ; push_int(sb, off + k * 8) ; push_str(sb, "(t0)\n")
    k = k + 1
  }
  cw
}
...
emit_rv_store_payload_at := fn(pe : ptr(Expr), off : i64, in out sb : rt::StrBuf, a : rt::Arena, src : ptr(u8), params_head : ptr(mut Param), pcount : i64, body_head : ptr(mut Stmt), decls : ptr(rt::Vec), bind_head : ptr(mut Bind), bind_base : i64) -> i64 {
  ...
  ecw := rv_store_enum_call_at(pe, off, sb, a, src, params_head, pcount, body_head, decls, bind_head, bind_base)
  if ecw != 0 { return ecw }
  emit_rv_expr(pe, sb, a, src, params_head, pcount, body_head, decls, bind_head, bind_base)
...
emit_rv_store_payload_atptr := fn(pe : ptr(Expr), off : i64, in out sb : rt::StrBuf, a : rt::Arena, src : ptr(u8), params_head : ptr(mut Param), pcount : i64, body_head : ptr(mut Stmt), decls : ptr(rt::Vec), bind_head : ptr(mut Bind), bind_base : i64) -> i64 {
  ...
  ecw := rv_store_enum_call_atptr(pe, off, sb, a, src, params_head, pcount, body_head, decls, bind_head, bind_base)
  if ecw != 0 { return ecw }
  emit_rv_expr(pe, sb, a, src, params_head, pcount, body_head, decls, bind_head, bind_base)
  push_str(sb, "  ld a1, 0(sp)\n  sd a0, ") ; push_int(sb, off) ; push_str(sb, "(a1)\n")
  return 1
...
emit_rv_sret_store := fn(e : ptr(Expr), in out sb : rt::StrBuf, a : rt::Arena, src : ptr(u8), params_head : ptr(mut Param), pcount : i64, body_head : ptr(mut Stmt), decls : ptr(rt::Vec), bind_head : ptr(mut Bind), bind_base : i64) {
  ...
  if enum_decl_of(decls, src, RV_RET_SRET_NS, RV_RET_SRET_NL) >= 0 {
    ...
    if (not edone) and expr_is_enum_lit(e) {
...
emit_rv_fn := fn(d : Decl, in out sb : rt::StrBuf, a : rt::Arena, src : ptr(u8), decls : ptr(rt::Vec)) {
  ...
  if rsbn.n != 0 and rv_standard_byte_abi_fence(decls, src, rsbn.s, rsbn.n, a) { standard_byte_abi_fence = true }
  if rv_params_need_standard_byte_abi_fence(ephead, src, decls, a) { standard_byte_abi_fence = true }
  ...
  if standard_byte_abi_fence { push_str(sb, "  ebreak # unsupported standard-byte by-value ABI boundary (#169)\n") }
  push_str(sb, "  addi sp, sp, -") ; push_int(sb, frame) ; push_str(sb, "\n")
...
pub emit_rv_program := fn(decls : ptr(rt::Vec), in out sb : rt::StrBuf, src : ptr(u8), src_n : usize, a : rt::Arena) {
  ...
  ## in `emit_rv_print_template`; `__print_u64` is untouched, so an unsigned hole keeps its bytes.
  ##
  ...
  ##
  ## `sub a0, zero, a0` on i64::MIN wraps back to i64::MIN, and that word read as UNSIGNED (divu) is
  ## exactly the magnitude 2^63 — so the minimum needs no special case and no widening. Getting that
  ## wrong is the obvious wrong fix here (`test/print_int_i64_min.al` records the x86_64 twin).
  ##
  ...
  ## always free. Numeric local labels 2:/3: — the rest of this backend's emission uses only 1:.
  if RV_PRINT_I64 {
    push_str(sb, ".text\n__print_i64:\n  la a3, .Lnumbuf\n  addi a4, a3, 24\n  mv a5, a4\n  li a6, 10\n  sb zero, 0(a3)\n")
    push_str(sb, "  bgez a0, 2f\n  li t0, 1\n  sb t0, 0(a3)\n  sub a0, zero, a0\n")
    push_str(sb, "2:\n  divu a7, a0, a6\n  mul t0, a7, a6\n  sub t0, a0, t0\n  addi t0, t0, 48\n  addi a5, a5, -1\n  sb t0, 0(a5)\n  mv a0, a7\n  bnez a0, 2b\n")
    push_str(sb, "  lbu t0, 0(a3)\n  beqz t0, 3f\n  li t0, 45\n  addi a5, a5, -1\n  sb t0, 0(a5)\n")
    push_str(sb, "3:\n  li a0, 1\n  mv a1, a5\n  sub a2, a4, a5\n  li a7, 64\n  ecall\n  ret\n")
  }
}
