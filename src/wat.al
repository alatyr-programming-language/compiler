...
## annotation to find at all, so `wat_local_ann_signed` above answers "not proven" and an enclosing
...
## Structurally the mirror image of `wat_operand_unsigned`'s `Bin` arm, down to the operator list;
  ...
  match deref(v) {
    Expr::Bin(op, bl, br) => {
      if op == 16 or op == 17 or op == 18 or op == 19 or op == 29 or op == 34 or op == 35 or op == 36 {
        sl := wat_operand_signed_dep(bl, params_head, body_head, src, a, dep)
        sr := wat_operand_signed_dep(br, params_head, body_head, src, a, dep)
        if sl and sr { r = true }
...
## backend's `:=` locals are function-scoped, `wat_local_rhs` resolves a name to its FIRST flat
...
wat_operand_signed_dep := fn(e : ptr(Expr), params_head : ptr(mut Param), body_head : ptr(mut Stmt), src : ptr(u8), a : rt::Arena, dep : i64) -> bool {
  ...
  match deref(e) {
    Expr::Var(s, n) => {
      ...
      if r == false { rhs := wat_local_rhs(body_head, src, s, n, a); if unchecked bitcast(usize, rhs) != 0 { if wat_shift_call_signed(rhs, params_head, body_head, src, a, dep + 1) { r = true } } }
...
## The UNSIGNED-direction twin of `wat_hole_index_signed` below. An `xs[i]` read carries no
...
## array FIELD of a struct (`r.u[0]`), whose base is an `Expr::Field` that only `wat_place_ty` can
...
wat_index_elem_unsigned := fn(bse : ptr(Expr), params_head : ptr(mut Param), body_head : ptr(mut Stmt), src : ptr(u8), a : rt::Arena) -> bool {
  ...
  mut r := false
  if bn != 0 {
    an := wat_local_arrty_span(body_head, src, bs, bn, a)
    if arrty_elem_unsigned(src, an.s, an.n) { r = true }
    if r == false { sp := wat_slice_param_elem_span(params_head, src, bs, bn) ; if sp.n != 0 { if scalar_name_is_unsigned(src, sp.s, sp.n) { r = true } } }
    ## A `Slice(T)` LOCAL. Its element type is written ONLY in the annotation at its declaration,
    ## so neither the fixed-array span above nor the param lookup can see it. `wat_slice_elem_span`
    ...
    ## `wat_local_ann_span` already walks to the DECLARATION and answers its annotation span, so the
    ...
    if r == false {
      la := wat_local_ann_span(body_head, src, bs, bn, a)
      if la.n > 6 {
...
wat_operand_unsigned := fn(e : ptr(Expr), params_head : ptr(mut Param), body_head : ptr(mut Stmt), src : ptr(u8), a : rt::Arena) -> bool {
  ...
  match deref(e) {
    ...
    Expr::Index(bse, ix) => { if wat_index_elem_unsigned(bse, params_head, body_head, src, a) { r = true } }
    Expr::Num | Expr::BoolLit | Expr::If | Expr::Match | Expr::StructLit | Expr::Field
...
##
## LAYERED ON TOP of `wat_operand_signed`, never folded into it. That predicate is the oracle `/`, `%` and
## `shr` route on, so widening IT moves ARITHMETIC selection, not just rendering: adding a single
## `Expr::Index` arm to it was MEASURED to turn `arr[0] / 2` from `i64.div_u` into `i64.div_s`, `arr[0] % 3` from
## `i64.rem_u` into `i64.rem_s` and `shr(arr[0], 1)` from `i64.shr_u` into `i64.shr_s`.
## Rendering must not buy its type recovery at that price, so the extra recovery lives here and is
## read at the ONE call site that chooses `$__itoa_s` vs `$__itoa`.
##
...
##
## ONE-DIRECTIONAL, and in the opposite direction from `wat_operand_unsigned`: it can only move a hole
...
wat_hole_index_signed := fn(bse : ptr(Expr), body_head : ptr(mut Stmt), src : ptr(u8), a : rt::Arena) -> bool {
  ...
  bn := ex_var_nl(bse)
  mut r := false
  if bn != 0 { an := wat_local_arrty_span(body_head, src, bs, bn, a) ; if arrty_elem_signed(src, an.s, an.n) { r = true } }
  r
}
...
mut WAT_BRK := 0
mut WAT_CONT := 0
## Whether the nearest loop's break target carries an i64 result. Statement loops leave this false;
## an Expr::Loop sets it true while its body is emitted so `break <expr>` can branch with a value.
mut WAT_BRK_VALUE := false
...
## wrong TEXT (#443). `wat_hole_signed` asks `wat_operand_signed` — the SAME oracle `/`, `%` and `shr`
## route on, so the renderer and the arithmetic agree about what the operand is — and then, ONLY for
...
emit_wat_expr := fn(e : ptr(Expr), in out sb : rt::StrBuf, a : rt::Arena, src : ptr(u8), params_head : ptr(mut Param), pcount : i64, body_head : ptr(mut Stmt), decls : ptr(rt::Vec), bind_head : ptr(mut Bind), bind_base : i64) {
  match deref(e) {
    ...
    Expr::Loop(lb) => {
      ...
      emit_wat_stmts(lb, body_head, true, false, sb, a, src, params_head, pcount, decls, bind_head, bind_base)
      ## The loop has no fall-through value: every normal iteration jumps back to its own label, while
      ## a value-break branches directly to the outer result block. Keep the outer block validator-safe
      ## with an explicit unreachable after the non-falling loop expression.
      push_str(sb, "    )\n    (br 0)\n  )\n  (unreachable)\n)")
      WAT_BRK = ob
      WAT_CONT = oc
      WAT_BRK_VALUE = obv
